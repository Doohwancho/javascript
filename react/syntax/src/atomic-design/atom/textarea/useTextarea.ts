...
export const UseTextArea = (textareaProps: TextAreaProps) => {
  ...
  const combinedFuncion = (
    //TODO - 이 함수형 파헤치기
    fnA: React.FormEventHandler<HTMLTextAreaElement>,
    fnB: React.FormEventHandler<HTMLTextAreaElement>
  ) => {
    const compose =
      (f: React.FormEventHandler<HTMLTextAreaElement>, g: any) =>
      (x: unknown) =>
        f(g(x));
  ...
  const getTextAreaProps = () => {
    ...
    if (isSmart) {
      textProps.onInput =
        typeof textareaProps.onInput === "function"
          ? combinedFuncion(handleResizeHeight, textareaProps.onInput)
          : handleResizeHeight;
    }
