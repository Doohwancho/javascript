...
import { getColorByToken, hexToRgba, uuidv4 } from "../theme";
import { ClearableIcon, VisibleIcon } from "../theme";
import { ControlClear, ControlVisible } from "../mixins";
...
const TextInput = (props: InputProps) => {
  const id = uuidv4();
  const { variant, inputSize, label, placeholder, append, clearable, visible } =
    props;
    ...
    <>
      <div className="input_wrapper" css={inputWrapperStyle}>
        ...
        {visible && props.type === "password" && (
          <ControlVisible type={type} setType={setType} />
        )}
      </div>
    </>
  );
