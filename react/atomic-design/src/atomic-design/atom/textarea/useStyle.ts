...
export const useStyle = (textareaProps: TextAreaProps) => {
  ...
  useEffect(() => {
    setMaxWidth(textareaProps.maxWidth ?? defaultStyle.maxWidth); //?? is used to provide a default value when dealing with null or undefined values.
    ...
    textareaProps.maxHeight,
    textareaProps.maxWidth,
    textareaProps.appearance,
    textareaProps.color,
  ]);
      ...
      return css({
        ...
        "&:hover": {
          backgroundColor: `${hexToRgba(
            getColorByToken(defaultHoverColorToken),
            0.5
          )}`,
        },
        "&:focus-visible": {
  ...
  const textAreaStyle = [
    styleTheme.defaultTextAreaStyle(),
    styleTheme.reSizeStyle(),
    styleTheme.sizeStyle(),
    styleTheme.appearanceStyle(),
  ];
