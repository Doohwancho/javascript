...
const BoardContent = () => {
  ...
  const recommendHandler = async () => {
    ...
    try {
      const res = await addRecommend(boardDetail.boardId, loggedInUser.accessToken); //TODO - 추천 역시 jwt 첨부해서 값을 따로 가져오는구만. 추천용 테이블이 분리되있진 않은 것 같고. 걍 select for update겠네. 
  ...
  return (
    <StyledBoardContent>
      ...
      <MarkdownViewer content={boardDetail.content} /> {/* content는 markdown-viewer 라이브러리 쓴 듯 */}
      <TagsAndRecommend>
        <Tags>
