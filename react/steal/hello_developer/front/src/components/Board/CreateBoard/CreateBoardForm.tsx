...
const CreateBoardForm = ({ tempBoardId }: CreateBoardFormProps) => {
  ...
  const changeTags = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    switch (name) {
      ...
      case "tag3":
        setPostData((prevState) => {
          return { ...prevState, tags: [postData.tags[0], postData.tags[1], { name: value }] }; //TODO - tag 처리를 이렇게 했구나?
        });
        break;
      default:
        break;
  ...
  const submitHandler = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      setIsLoading(true); //TODO - post 전 loading 먼저 띄우기 
      const res = await createBoard(loggedInUser.accessToken, { title, category, tags, content, tempBoardId }); //TODO - create board post 요청시, uuid도 보내는구나? 근데 db에서 id insert시 auto_increment 쓰지 않나?
