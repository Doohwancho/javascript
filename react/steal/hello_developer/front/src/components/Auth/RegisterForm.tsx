...
const RegisterForm = () => {
  ...
  const submitHandler = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err.response.status !== 500) { //TODO - server validation(2중 체크)
  ...
  const accountChangeHandler = (event: ChangeEvent<HTMLInputElement>) => { //TODO - server validation 전, client side validation on e.change
  ...
  return (
    <StyledRegisterForm onSubmit={submitHandler}>
      <FormControl>
        ...
        {isValidAccount.email !== null && !isValidAccount.email && account.email.length !== 0 && ( //TODO - if validation fails, show this view component
