...
const BoardRegister: React.FC = (props: any) => {
  ...
  const token = useSelector((state: any) => state.Auth.token); //TODO - state management에서 보관하던 jwt token을 가져와 jwtUtil lib로 파싱해서 user-id를 첨부해 보낸다. 
  ...
  const addBoard = async (board: Board) => {
    ...
    props.history.push('/board-list'); //TODO - 뒤로가기 처리
  }
