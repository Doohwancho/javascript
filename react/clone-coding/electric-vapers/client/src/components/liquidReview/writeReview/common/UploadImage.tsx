...
const UploadImage = () => {
  ...
  const imageUploadHandler = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      for (let i = 0; i < files.length; i++) { //TODO - validation 모두 끝난 다음, 이미지 업로드 허용하는게 좋을 것 같다. 중간에 업로드 됬다가, 막히면, undo하는 로직 또 짜기 보다는. 
        ...
        reader.onload = (e: any) => {
          setBlobImages((blobImages) => [...blobImages, e.target.result]); //TODO - image upload 하기 
        };
